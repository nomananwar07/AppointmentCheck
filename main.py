...
def Check_appointment():
    ...
    headers = {
        "Cookie": "tvo_session={}; tvo_cookie_accept=1".format(c),
    ...
    if content.body.find(text='Kein freier Termin verfügbar'):
        ...
        url = "https://termine.staedteregion-aachen.de/auslaenderamt/location?mdt=78&select_cnc=1&cnc-198=1"
        r = requests.post(url, headers=headers, params=payload, cookies=cookie)
        print (r.status_code)
        # print (r.url)



        r = requests.get("https://termine.staedteregion-aachen.de/auslaenderamt/suggest", cookies=cookie,headers=headers)
        print (r.status_code)
        # print (r.url)
